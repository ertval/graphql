  ...
  await page.route("**/api/graphql-engine/v1/graphql", async (route) => {
    ...
    if (query.includes("GetXPTransactions")) {
      await route.fulfill({
        status: 200,
        contentType: "application/json",
        body: JSON.stringify({ data: { transaction: mockGraphqlData.xpTransactions } }),
      });
      return;
    }
...
test("login form enforces required fields at DOM/runtime level", async ({ page }) => {
  await page.goto("/");

  await expect(page.locator("#identifier")).toHaveAttribute("required", "");
  await expect(page.locator("#password")).toHaveAttribute("required", "");

  const formValidityBefore = await page.$eval("#login-form", (form) => form.checkValidity());
...
test("logout returns to login and clears credential inputs", async ({ page }) => {
...
test("XP by Project interaction opens project detail modal", async ({ page }) => {
  await loginWithMockBackend(page);
  ...
  await expect(page.locator("#project-detail-overlay")).not.toHaveClass(/active/);
});

test("main nav switches between dashboard and collaborations tab", async ({ page }) => {
  await loginWithMockBackend(page);

  await page.click("#tab-collaborations");
  await expect(page.locator("#tab-collaborations")).toHaveAttribute("aria-selected", "true");
